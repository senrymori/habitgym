...
export const HabitForm: FC<HabitFormProps> = function (props) {
  ...
  const handleHabitTypeChange = useCallback(
    (index: number) => props.form.setValue('habitType', habitTypeSegments[index], { shouldValidate: true, shouldDirty: true }),
    [props.form]
  );
  const handleTrackingModeChange = useCallback(
    (index: number) => props.form.setValue('trackingMode', trackingModeSegments[index], { shouldValidate: true, shouldDirty: true }),
    [props.form]
  );
  ...
  const showWeekdayPicker =
    habitType === 'weekly' || (habitType === 'tracking' && trackingMode === 'weekly');
        ...
        {showWeekdayPicker ? (
          <View style={sharedLayoutStyles.gap8}>
            ...
            <Controller
              ...
              rules={{
                validate: (v) =>
                  v.length >= 1 || translations.habits.validation.daysRequired,
              }}
              render={({ field, fieldState }) => (
